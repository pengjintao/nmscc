...
NMS_API int Mutex::init()  {
    const auto stat = mtx_init(&_fobj, 0);
    if (stat != 0) {
...
NMS_API int Mutex::destroy() {
    const auto stat =  mtx_destroy(&_fobj);
    if (stat != 0) {
...
NMS_API void Mutex::lock() noexcept {
    const auto stat = mtx_lock(&_fobj);
    if (stat != 0) {
...
NMS_API bool Mutex::trylock() {
    const auto stat =  mtx_trylock(&_fobj);
    return stat == 0;
}

NMS_API void Mutex::unlock() noexcept {
    const auto stat = mtx_unlock(&_fobj);
    if (stat != 0) {
